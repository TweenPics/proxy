...
module.exports = ( { cert, handle, isHandled, key, port } ) => {
	...
	const httpPromise = new Promise( resolve => http.createServer( ( request, response ) => {
		let target = `http://${request.headers.host}`;
		if ( isHandled( target ) ) {
			target = handle( request, response );
		}
		if ( target ) {
			passthrough.web( request, response, { target } );
		}
	...
	const httpsPromise = new Promise( resolve => https.createServer( { key, cert }, ( request, response ) => {
	...
	return Promise.all( [ httpPromise, httpsPromise ] ).then( () => `localhost:${port}` );
};
