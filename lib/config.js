...
module.exports = new Promise( resolve =>
    require( `james-browser-launcher` ).detect( available => {
        const availableSet = new Set( available.map( info => info.name ) );
        if ( !availableSet.has( config.browser ) ) {
            const listOr = require( `./listOr` );
            const browsers = Array.from( availableSet, name => `"${ name }"` );
            browsers.sort();
            console.error( `browser "${ config.browser }" is not a valid browser name (not ${ listOr( browsers ) })` );
            process.exit( 2 );
        }
        resolve( config );
    } )
